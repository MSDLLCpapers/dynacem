...
You can install the development version of *dynacem* from [GitHub](https://github.com/) as follows.
...
The package comes with a cost-effectiveness model object, *oncpsm*, created using [heemod](https://cran.r-project.org/package=heemod).
...
The next steps extract a payoff vector from the model object. The model/object contains several payoffs accumulated in each timestep, calculated as at time zero: 
...
The *get_dynfields()* function extracts these parameters from the [heemod](https://cran.r-project.org/package=heemod) model object, and calculates 'rolled-up' values as at the start of each timestep rather than discounted to time zero. The rolled-up values are what *dynacem* requires.
...
The total uptake is `r format(round(pv1$results$uptake, 0), big.mark=",")` patients with a total present value of `r format(round(pv1$results$total, 0), big.mark=",")`. This equates to a mean present value per patient of `r format(round(pv1$results$mean, 0), big.mark=",")`.

### Present values into the future

We also wish to calculate discounted present values (PV) into the future, say every annually for 10 years, assuming a single cohort.
