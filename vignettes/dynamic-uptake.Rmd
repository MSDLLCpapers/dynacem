...
There are `r sum(uptakes2)` patients with a total NPV of `r total(pv2)`, which equates to `r mean(pv2)` on average per patient. This can also be calculated in base R.
...
There are `r sum(uptakes3)` patients with a total NPV of `r total(pv3)`, which equates to `r mean(pv3)` on average per patient. Verifying this result in base R also becomes more complicated.
...
There are `r sum(uptakes3)` patients with a total NPV of `r total(pv4)`, which equates to `r mean(pv4)` on average per patient.
