...
```{r dynpricing}
# Dates
doc <- lubridate::ymd("20250901") # Date of calculation = 1 September 2025
loe_soc_start <- lubridate::ymd("20280101") # Date of LOE for SoC = 1 January 2028
loe_soc_end <- lubridate::ymd("20290101") # Maturation of SoC prices by LOE + 1 year, i.e. = 1 January 2029
loe_new_start <- lubridate::ymd("20310101") # Date of LOE for new treatment = 1 January 2031
loe_new_end <- lubridate::ymd("20320101") # Maturation of new treatment prices by LOE + 1 year, i.e. = 1 January 2032
...
# Create a tibble of price indices of length T, then pull out columns as needed
pricetib <- dplyr::tibble(
  model_time = 1:Ncycles,
  model_year = model_time * cycle_years,
  static = 1,
  geninfl = (1 + infl_cycle)^(model_time - 1),
  loef_soc = pmin(pmax(model_time - wk_start_soc, 0), wk_mature_soc-wk_start_soc) / (wk_mature_soc-wk_start_soc),
  loef_new = pmin(pmax(model_time - wk_start_new, 0), wk_mature_new-wk_start_new) / (wk_mature_new-wk_start_new),
...
s1_soc_othcost <- dynamicpv::dynpv(
    ...
    payoffs = hemout_soc$cost_nondaq,
    prices = prices_oth,
    discrate = nomdisc_cycle
    )$results$total
s1_soc_daqcost <- dynamicpv::dynpv(
    uptakes = uptake_single,
    payoffs = hemout_soc$cost_daq_soc,
...
s1_new_othcost <- dynamicpv::dynpv(
    uptakes = uptake_single,
    payoffs = hemout_new$cost_nondaq,
    prices = prices_oth,
    discrate = nomdisc_cycle
    )$results$total
s1_new_daqcost <- dynamicpv::dynpv(
    uptakes = uptake_single,
    payoffs = hemout_new$cost_daq_new,
    prices = prices_static,
    discrate = disc_cycle
    )$results$total
...
These results show that the new intervention is associated with $`r s1_icost` incremental costs relative to the standard of care (including $`r s1_new_daqcost` of drug acquisition costs for the new intervention) and `r s1_iqaly` incremental QALYs. The cumulative ICER (incremental cost per QALY) starts high but decreases over time towards $`r s1_icer` per QALY at the 20 year time horizon.
...
- The effect of assuming dynamic pricing rather than flat pricing over time is shown in the difference between results of scenarios 1 and 2. Scenario 1 assumed no increases in drug acquisition costs, but inflationary increases in other costs. Scenario 2 additionally assumed that the price of the new intervention would be eroded by 70% by week 70. This has a material impact on costs per patient of the new intervention (reducing from $`r s1_new_cost` to $`r s2_new_cost`). Accordingly the ICER reduces from $`r s1_icer` to $`r s2_icer` per QALY.

- The effect of modeling future cohorts rather than just the current cohort of patients is shown in the difference between results of scenarios 1 and 3. Scenario 3 assumed regular, flat incidence of new patients, with uptake increasing gradually to 100% over 60 weeks. This has the effect of weighting the costs and QALYs per patient in scenario 3 towards a later cohort of patients, where the accumulation of costs and QALYs over the payer's remaining 20 year time horizon will be less. The incremental costs per patient reduced from $`r s1_new_cost` in scenario 1 to $`r s3_new_cost/sum(uptake_multi)` in scenario 3, whereas the incremental QALYs reduced from `r s1_new_qaly` to `r s3_new_qaly/sum(uptake_multi)` per patient. Overall, the ICER increased slightly from $`r s1_icer` to $`r s3_icer` per QALY.

- Scenario 4 illustrates the effects of both dynamic pricing and cohort modeling. The results per patient are identical to scenario 3, execpt that the cost per patient of the new intervention reduces from $`r s3_new_cost/sum(uptake_multi)` to $`r s4_new_cost/sum(uptake_multi)`, due to the impact of dynamic pricing. The ICER in this case is reduced considerably to $`r s4_icer` per QALY.