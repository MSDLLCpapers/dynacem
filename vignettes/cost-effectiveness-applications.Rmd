...
There are now a growing number of publications in which multi-cohort cost-effectiveness evaluations with life cycle pricing have been presented.[@puls_2024; @schottler_2023; @whittington_2025a; @shafrin_2024; @moreno_2016] The purpose of this R package is to provide a simple tool to conduct calculations of present values that allow for dynamic pricing and dynamic uptake. This vignette aims to illustrate how computations may be performed. It is intended to be read after `vignette("dynamic-pricing")` and `vignette("dynamic-uptake")`. A mathematical framework presented here formalizes what others have developed and applied,[@hoyle_anderson_2010;@shafrin_2024] and provides the technical basis of the calculations within the package.

## Mathematical framework

Let us partition time as follows. Suppose $j=1, ..., T$ indexes the time at which the patient begins treatment (with either the new intervention of Standard of Care, SoC), where $T$ is the time horizon of the decision-maker. Suppose $k=1, ..., T$ indexes time since initiating treatment.

This can be illustrated through an example. Suppose then we are considering a cashflow in timestep 3. This will comprise:
...
The total present value is therefore the sum over all $j$ and $k$ within the time horizon $T$, namely:

$$ TPV = \sum_{j=1}^{T} \sum_{k=1}^{T-j+1} PV(j, k) = \sum_{j=1}^{T} \sum_{k=1}^{T-j+1} u_j \cdot p_k \cdot R_{j+k-1} \cdot (1+i)^{2-j-k} $$

The `dynamicpv::dynpv()` function operationalizes this calculation with arguments:

- `payoffs` $= {p_k}$
- `uptakes` $= {u_j}$
- `horizon` $= T$, defaulting to the length of `payoffs`
- `prices` $= {R_t}$, where $t=j+k-1+t_0$
- `discrate` $= i$
- `tzero` $= t_0$, defaulting to zero

The `tzero` argument is a time offset useful to be able to calculate present values into the future, which can be performed for single cohorts by `dynamicpv::futurepv()`.
...
This model may then be coded in [heemod](https://cran.r-project.org/package=heemod) as follows.
...
Let us suppose the following assumptions concerning patient uptake. The aim here is to estimate the incidence of patients for whom the decision problem applies, i.e. the patients who would receive the new intervention, were it made available.
...
In this way, uptake would be gradually increasing with time, accounting for disease epidemiology and the share of patients who receive the new intervention. The assumptions can be codified as follows.
...
# Incrementals
s1_icost <- s1_new_cost@total - s1_soc_cost@total
s1_iqaly <- s1_new_qaly@total - s1_soc_qaly@total
s1_icer <- s1_icost / s1_iqaly
```

These results show that the new intervention is associated with $`r s1_icost` incremental costs relative to the standard of care (including $`r s1_new_daqcost@total` of drug acquisition costs for the new intervention) and `r s1_iqaly` incremental QALYs. The cumulative ICER (incremental cost per QALY) is $`r s1_icer` per QALY at the 20 year time horizon.
...
# Incrementals
s2_icost <- s2_new_cost@total - s2_soc_cost@total
s2_iqaly <- s2_new_qaly@total - s2_soc_qaly@total
...
# Incrementals
s3_icost <- s3_new_cost@total - s3_soc_cost@total
s3_iqaly <- s3_new_qaly@total - s3_soc_qaly@total
...
# Incrementals
s4_icost <- s4_new_cost@total - s4_soc_cost@total
s4_iqaly <- s4_new_qaly@total - s4_soc_qaly@total
...
| Total costs (cohort) | | ||||
| | New intervention | `r s1_new_cost@total` | `r s2_new_cost@total` | `r s3_new_cost@total` | `r s4_new_cost@total` |
| | Standard of care | `r s1_soc_cost@total` | `r s2_soc_cost@total` | `r s3_soc_cost@total` | `r s4_soc_cost@total` |
| | Incremental | `r s1_icost` | `r s2_icost` | `r s3_icost` | `r s4_icost` |
| Total costs (per patient) | | ||||
| | New intervention | `r s1_new_cost@total` | `r s2_new_cost@total` | `r s3_new_cost@total/sum(uptake_multi)` | `r s4_new_cost@total/sum(uptake_multi)` |
| | Standard of care | `r s1_soc_cost@total` | `r s2_soc_cost@total` | `r s3_soc_cost@total/sum(uptake_multi)` | `r s4_soc_cost@total/sum(uptake_multi)` |
| | Incremental | `r s1_icost` | `r s2_icost` | `r s3_icost/sum(uptake_multi)` | `r s4_icost/sum(uptake_multi)` |
| Total QALYs (cohort) | ||||
| | New intervention | `r s1_new_qaly@total` | `r s2_new_qaly@total` | `r s3_new_qaly@total` | `r s4_new_qaly@total` |
| | Standard of care | `r s1_soc_qaly@total` | `r s2_soc_qaly@total` | `r s3_soc_qaly@total` | `r s4_soc_qaly@total` |
| | Incremental | `r s1_iqaly` | `r s2_iqaly` | `r s3_iqaly` | `r s4_iqaly` |
| Total QALYs (per patient) | ||||
| | New intervention | `r s1_new_qaly@total` | `r s2_new_qaly@total` | `r s3_new_qaly@total/sum(uptake_multi)` | `r s4_new_qaly@total/sum(uptake_multi)` |
| | Standard of care | `r s1_soc_qaly@total` | `r s2_soc_qaly@total` | `r s3_soc_qaly@total/sum(uptake_multi)` | `r s4_soc_qaly@total/sum(uptake_multi)` |
...
ds <- dplyr::tibble(
    ...
    # Scenario 1/2 QALYs
    qaly_soc = s1_soc_qaly@total,
    qaly_new = s1_new_qaly@total
  ) |>
  dplyr::mutate(
...
- The effect of assuming dynamic pricing rather than flat pricing over time is shown in the difference between results of scenarios 1 and 2. Scenario 1 assumed no increases in drug acquisition costs, but inflationary increases in other costs. Scenario 2 additionally assumed drug prices of the new intervention and SoC comparator would be eroded due to losses of exclusivity. This has a material impact on costs per patient of the new intervention (reducing from $`r s1_new_cost@total` to $`r s2_new_cost@total`). Accordingly the ICER reduces from $`r s1_icer` to $`r s2_icer` per QALY.

- The effect of modeling uptake dynamically rather than just the current cohort of patients is shown in the difference between results of scenarios 1 and 3. Dynamic modeling of uptake has the effect of weighting the costs and QALYs per patient in scenario 3 towards a later cohort of patients, where the accumulation of costs and QALYs over the time remaining in the payer's 20 year time horizon will be less. The incremental costs per patient reduced from $`r s1_new_cost@total` in scenario 1 to $`r s3_new_cost@total/sum(uptake_multi)` in scenario 3, whereas the incremental QALYs reduced from `r s1_new_qaly@total` to `r s3_new_qaly@total/sum(uptake_multi)` per patient. Overall, the ICER increased slightly from $`r s1_icer` to $`r s3_icer` per QALY.

- The effects of dynamic pricing and dynamic uptake are extremely synergistic. Scenario 4 illustrates the effects of both dynamic pricing and cohort modeling. The results per patient are identical to scenario 3, except that the cost per patient of the new intervention reduces from $`r s3_new_cost@total/sum(uptake_multi)` to $`r s4_new_cost@total/sum(uptake_multi)`, due to the impact of dynamic pricing. The ICER in this case is reduced considerably to $`r s4_icer` per QALY.
